...
type Props = {
  ...
  mode: Mode;
  onClose: () => void;
  onSave: (tom: TomItem, mode: 'create' | 'edit') => void;
};
...
const TomsDetailsDrawer: React.FC<Props> = ({ tom, isOpen, mode, onClose, onSave }) => {
      ...
      <div className="flex h-full w-full max-w-2xl flex-col overflow-y-auto bg-white shadow-2xl" ref={panelRef}>
        ...
        <div className="sticky bottom-0 border-t border-slate-200 bg-white p-4">
          <div className="flex justify-end gap-2">
            ...
            {isEditable && (
              <Button onClick={handleSave}>{mode === 'create' ? 'Create measure' : 'Save changes'}</Button>
            )}
          </div>
        </div>
      </div>
